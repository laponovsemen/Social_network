...
export class AuthController {
    ...
    async login(
        ...
        const newSession =
            await this.securityDevicesRepository.createNewSession(
                user,
                ip,
                deviceName,
                deviceId,
    ...
    async registrationConfirmation(
        ...
        const result =
            await this.authService.registrationConfirmation(codeFromQuery);
        if (!result) {
            res.status(400).json({
                errorsMessages: [
    ...
    @Post("registration")
    @HttpCode(HttpStatus.NO_CONTENT)
    async registration(@Res() res: Response, @Body() userDTO: UserDTO) {
    ...
    @HttpCode(HttpStatus.NO_CONTENT)
    async registrationEmailResending(@Res() res: Response, @Body() emailDTO) {
        console.log(emailDTO, " email in registrationEmailResending");
        const result =
            await this.authService.registrationEmailResending(emailDTO);
    ...
    async getProfile(
        ...
        const refreshTokenValidation =
            this.authService.verifyRefreshToken(accessToken);
