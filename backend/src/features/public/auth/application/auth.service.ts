...
import { EmailAdapter } from "../../notification/email.adapter";
import { Common } from "../../../common";
import { emailDTO, UserDTO } from "../../../input.classes";
import { ObjectId } from "mongodb";
import { SecurityDevicesRepository } from "../../public/security.devices/security.devices.repository";
...
export class AuthService implements OnModuleInit {
    ...
    async onModuleInit() {
        const token = await this.jwtService.signAsync(
            { userId: randomUUID(), deviceId: randomUUID() },
    ...
    async signIn(user: any, ip: string, title: string, deviceId: string) {
        ...
        return {
            access_token: await this.jwtService.signAsync(payload, {
                expiresIn: this.timeOfLivingAccessToken,
                secret: jwtConstants.secretForAccess,
            }),
            refresh_token: await this.jwtService.signAsync(payload, {
                expiresIn: this.timeOfLivingRefreshToken,
    ...
    async registrationEmailResending(emailFromFront: emailDTO) {
        ...
        if (!UserExists) {
            ...
            await this.usersRepository.changeUsersConfirmationCode(
                UserExists.id,
                confirmationCode,
            );
            return {
    ...
    async registrationConfirmation(code: string) {
        const foundUser =
            await this.usersRepository.findUserByRegistrationCode(code);
        ...
        const foundUserCodeFreshness =
            await this.usersRepository.findUserCodeFreshness(foundUser);
    ...
    async refreshToken(refreshToken: string) {
        ...
        const foundDevice =
            await this.securityDevicesRepository.gedDeviceByDeviceId(deviceId);
        ...
        const payload = {
            userId: refreshTokenVerification.userId,
        ...
        const updatedSession =
            await this.securityDevicesRepository.updateSessionByDeviceId(
                deviceId,
                lastActiveDate,
                newRefreshToken,
            );
