...
import { LikeRepository } from "../../../likes/infrastructure/likes.repository";
...
export class BanProcedureUseCase
    implements ICommandHandler<BanProcedureCommand>
{
    constructor(
    ...
    async execute(command: BanProcedureCommand) {
        ...
        console.log("start execute of BanProcedureCommand");

        if (!newBanStatusOfUser) {
            console.log("try to ban user");
            ...
            return await this.usersRepository.unbanUserDB(
                command.userId,
                command.DTO,
            );
        } else {
            console.log("try to unban user");
            ...
            return await this.usersRepository.banUserDB(
                command.userId,
                command.DTO,
            );
        }
