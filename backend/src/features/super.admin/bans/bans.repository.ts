...
export class BansRepository {
    constructor(
        ...
        @InjectRepository(BloggerBansForSpecificBlog)
        protected bloggerBansForSpecificBlogTypeORMRepository: Repository<BloggerBansForSpecificBlog>,
        @InjectRepository(Blog)
        protected blogsTypeORMRepository: Repository<Blog>,
    ) {}

    async banUserForSpecificBlog(
        ownerId: string,
        userToBanId: string,
        DTO: BanUserByBloggerDTO,
    ) {
        const blogId = DTO.blogId;
        const userToBan: User =
            await this.usersRepository.findUserById(userToBanId);
        ...
        const banExists: BloggerBansForSpecificBlog =
            await this.bloggerBansForSpecificBlogTypeORMRepository.findOne({
                where: {
                    bannedUser: {
                        id: userToBanId,
                    },
        ...
        const blog: Blog = await this.blogsTypeORMRepository.findOneBy({
            id: blogId,
        });
        ...
        if (banExists && DTO.isBanned) {
            ...
            const newBan = BloggerBansForSpecificBlog.create(
                owner,
                userToBan,
                blog,
                DTO,
            ...
            const res =
                await this.bloggerBansForSpecificBlogTypeORMRepository.save(
                    newBan,
                );
    ...
    async getAllBannedUsersForSpecificBlog(
        ...
        const totalCount =
            await this.bloggerBansForSpecificBlogTypeORMRepository.countBy({
                isBanned: true,
                blog: {
                    id: blogId,
                },
        ...
        console.log(
            totalCount,
            "totalCount in bloggerBansForSpecificBlogTypeORMRepository",
        );
    ...
    const result =  await request
        ...
        const result =
            await this.bloggerBansForSpecificBlogTypeORMRepository.find({
                relations: {
                    bannedUser: true,
                    blog: true,
                    owner: true,
    ...
    const result = findBanStatusQuery[0]*/
        ...
        const result =
            await this.bloggerBansForSpecificBlogTypeORMRepository.findOneBy({
                blog: {
                    id: blogId,
                },
                bannedUser: {
