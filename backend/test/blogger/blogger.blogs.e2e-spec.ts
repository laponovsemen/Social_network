...
describe("TESTING OF CREATING USER AND AUTH", () => {
    ...
    it("create user, login, create blog, create another user , create posts for specific blog, comment them and get them", async () => {
        ...
        const loginRes1 = await request(server).post("/auth/login").send({
            loginOrEmail: user0.login,
            password: user0.password,
        });
    ...
    it("create user, login, create blog, create another user , create posts for specific blog, comment them and ", async () => {
        ...
        await request(server)
            .put(
                `/blogger/blogs/${blogId1}/posts/602afe92-7d97-4395-b1b9-6cf98b351bbe`,
            )
            .auth(loginRes1.body.accessToken, { type: "bearer" })
            .send({
                content: "content after update",
    ...
    it("create user, login, create blog, create another user and ban user2 for blog", async () => {
        ...
        const uploadMainPhotoForBlog = await request(server)
            ...
            .attach(
                ...
                path.join(
                    __dirname,
                    "../testing.images/testing-image-156x156pixels.jpg",
                ),
            );
        ...
        const uploadWallpaperPhotoForBlog = await request(server)
            ...
            .attach(
                ...
                path.join(
                    __dirname,
                    "../testing.images/blogs-wallpaper1028x312.jpg",
                ),
            );
        ...
        const updateImageForPost = await request(server)
            .post(
                `/blogger/blogs/${blogId}/posts/${createPostForSpecificPost.body.id}/images/main`,
            )
            .auth(accessToken, { type: "bearer" })
            .attach(
                "file",
                path.join(__dirname, "../testing.images/main-post940x432.jpg"),
            )
            .expect(201);
