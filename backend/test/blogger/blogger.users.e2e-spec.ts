...
describe("TESTING OF CREATING USER AND AUTH as", () => {
    ...
    it("create user, login, create blog, create another user and ban user2 for blog", async () => {
        ...
        console.log(
            { loginOrEmail: res.body.login, password: "passwordpassword" },
            "login data",
        );
        console.log(res.body.id, " res.body.id");

        const loginMainRes = await request(server).post("/auth/login").send({
        ...
        expect(
            allUsersAfterUnban.body.items.find(
                (user) => user.id === userIdToUnban,
            ),
        ).toBeUndefined();
        ...
        expect(
            allUsersAfterUnban2.body.items.find(
                (user) => user.id === allUsersAfterUnban.body.items[0].id,
            ),
        ).toBeUndefined();

        await request(server)
            .put(
                `/blogger/users/${allUsersAfterUnban.body.items[allUsersAfterUnban.body.items.length - 1].id}/ban`,
            )
            .auth(accessToken, { type: "bearer" })
            .send({
                isBanned: false,
        ...
        expect(
            allUsersAfterUnban2.body.items.find(
                (user) =>
                    user.id ===
                    allUsersAfterUnban.body.items[
                        allUsersAfterUnban.body.items.length - 1
                    ].id,
            ),
        ).toBeDefined();

        console.log(
            allUsersAfterUnban2.body.items,
            " allUsersAfterUnban2.body.items",
        );

        expect(
            allUsersAfterUnban3.body.items.find(
                (user) =>
                    user.id ===
                    allUsersAfterUnban.body.items[
                        allUsersAfterUnban.body.items.length - 1
        ...
        console.log(
            allUsersAfterUnban3.body.items,
            " allUsersAfterUnban3.body.items",
        );
    }, 30000);
