...
describe("TESTING OF CREATING USER AND AUTH", () => {
    ...
    it(
        "should create user by super admin, login," +
            " create blog and post for this blog ," +
            " ban blog and try to get post of banned blog",
        async () => {
            ...
            await request(server)
                .put(
                    `/blogger/blogs/2281331/posts/${createdPostForSpecificBlog.body.id}`,
                )
                .set("Authorization", `Bearer ${accessTokenOfUser}`)
                .send({
                    title: "string2",
            ...
            await request(server)
                .put(
                    `/blogger/blogs/${createdBlog.body.id}/posts/${createdPostForSpecificBlog.body.id}`,
                )
                .set("Authorization", `Bearer ${accessTokenOfUser}`)
                .send({
                    title: "string2",
