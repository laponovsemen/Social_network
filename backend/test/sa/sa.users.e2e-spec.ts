...
describe("TESTING OF CREATING USER AND AUTH", () => {
    ...
    it("shoud create user , ban it by SA and try to login => result must be 401", async () => {
        ...
        expect(loginProcedure.body).toEqual({
        ...
        console.log(
            accessTokenOfUser,
            " blogId accessTokenOfUser created blog",
        );
    ...
    it("shoud create user , create blog and try to delete blog", async () => {
        ...
        const accessTokenOfUser = loginProcedure.body.accessToken;
        const accessTokenOfUserNumberTwo =
            loginProcedureOfUserTwo.body.accessToken;
    ...
    it("shoud create user , create blog and ban blog sf", async () => {
        await request(server).delete("/testing/all-data");
        const user = await request(server)
            .post("/sa/users")
            .set(authE2eSpec, basic)
        ...
        const getAllUsersBeforeBan = await request(server)
            .get(`/sa/users`)
            .set(authE2eSpec, basic)
            .expect(200);
        expect(getAllUsersBeforeBan.body).toEqual({
            items: [
                {
                    banInfo: {
                        banDate: null,
        ...
        const userToBan = getAllUsersBeforeBan.body.items[0];
        console.log(userToBan);

        const BanOfUser = await request(server)
            .put(`/sa/users/${userToBan.id}/ban`)
            .set(authE2eSpec, basic)
            .send({
                isBanned: true,
                banReason: "stringstringstringst",
        ...
        const getAllUsersAfterBan = await request(server)
            .get(
                `/sa/users?pageSize=15&pageNumber=1&sortDirection=asc&banStatus=banned`,
            )
            .set(authE2eSpec, basic)
            .expect(200);
        expect(getAllUsersAfterBan.body).toEqual({});
    }, 20000);
});
