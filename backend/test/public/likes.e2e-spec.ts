...
describe("CREATEING COMMENTS FOR SPECIFIED POST TESTFLOW", () => {
    ...
    it("should create user, blog, pot, comment , auth and get comments //auth is correct", async () => {
        ...
        const loginRes0 = await request(server).post("/auth/login").send({
        ...
        for (let i = 0; i < 10; i++) {
            ...
            await request(server)
                .put(
                    `/comments/602afe92-7d97-4395-b1b9-6cf98b351bbe/like-status`,
                )
                .auth(accessToken0, { type: "bearer" })
                .send({
                    likeStatus: "Like",
        ...
        expect(getAllPostForSpecificBlog.body).toEqual(
            resultForGettingAllPosts,
        );
    }, 60000);
});
