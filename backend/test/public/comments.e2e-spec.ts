...
describe("CREATING COMMENTS FOR Likes procedures testing", () => {
    ...
    it("should create user, blog, pot, comment , auth and get comments //auth is correct", async () => {
        ...
        await request(server).get(`/comments/2281337`).expect(404);
        await request(server).get(`/comments/${commentId}`).expect(200);
        ...
        const likedCommentAfterUpdatethroughPostCotroller = await request(
            server,
        )
